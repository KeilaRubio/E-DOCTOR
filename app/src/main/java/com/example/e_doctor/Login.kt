...
//    internal var relativeLayoutProgress: RelativeLayout? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mTextViewError=findViewById(R.id.text_view_error_pass_vacio)
        askPermissions()
...
//        }
        ...
        tvOlvidoContrasena.setOnClickListener {
            val intent = Intent(applicationContext, RecuperarContrasenaCorreo::class.java)
            startActivity(intent)
            overridePendingTransition(R.anim.enter, R.anim.exit)
        }
    }
    fun camposVacios(): Boolean {
        ...
        if (clave.isEmpty()) {
            ...
            mTextViewError.setText(getText(R.string.login_campo_obligatorio))
            retorna = true
        } else
...
//            permissionsNeeded.add("\n- Memoria: Permisos para acceder  a la memoria del teléfono.")
    ...
    fun recuperarContrasena(view: View) {
        val intent = Intent(this, RecuperarContrasenaCorreo::class.java)
                ...
                login.enqueue(object : Callback<API_LOGIN> {
                    override fun onResponse(call: Call<API_LOGIN>, response: Response<API_LOGIN>) {
                        if (response.isSuccessful) {//200 o 201
                            ...
                            if (response.body()!!.data.usuario.perfil.fecha_nacimiento != null) {
                                fecha_nacimiento = response.body()!!.data.usuario.perfil.fecha_nacimiento
                            }

                            mUsuario = Usuario(
                                nombre,
                                response.body()!!.data.usuario.perfil.genero,
...
//                                "", //este campo lo obtengo al entrar al main
                                response.body()!!.data.usuario.id_seguimiento
                            )
